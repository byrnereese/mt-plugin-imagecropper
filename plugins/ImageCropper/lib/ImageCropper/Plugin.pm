...
sub gen_thumbnails_start {
    ...
    my @protos;
    my @custom = MT->model('thumbnail_prototype')->load({
            blog_id => $app->blog->id
    });
    foreach (@custom) {
        push @protos, {
...
sub delete_crop {
    ...
    if ($oldmap) {
        my $oldasset
            = MT->model('asset')->load( $oldmap->cropped_asset_id );
        $oldasset->remove()
            or MT->log({    blog_id => $blog->id,
...
sub crop {
    ...
    $fmgr->put_data(
        $data, 
        File::Spec->catfile( $cache_path, $cropped ), 'upload'
    )
        or $error = MT->translate( "Error creating cropped file: [_1]",
                                   $fmgr->errstr );
